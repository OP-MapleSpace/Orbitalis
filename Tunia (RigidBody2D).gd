...
const period = 25; # years it takes for the planet to complete one orbit
...
func _ready():
	set_mass(1.1839*(10^26)) # mass in kg
...
func _physics_process(delta):
	# Orbital angle
	theta -= delta * sqrt(G / pow(radius, 3.0))*2000 * pow(10,5.5) # radians, scaled up to make it practical for the game
	...
	set_position(pos)
func _draw():
	draw_circle(Vector2(0,0), 10, Color(0.0, 0.0, 1.0));
